...
// Reads an event's description and adds a few relevant extra questions to
// the signup form — e.g. mentioning "3v3" prompts a half-court question,
// "tournament" prompts a bracket-experience question. Keyword-based, not a
// model call, so it's instant and free — but it means the questions only
// cover what's matched below.
const DYNAMIC_QUESTION_RULES = [
  { test: /\b3\s*v\s*3|3[\s-]on[\s-]3|three[\s-]on[\s-]three\b/i, q: { id: 'q_3v3', label: 'Comfortable playing 3v3 half-court?', type: 'yesno' } },
  { test: /\b5\s*v\s*5|5[\s-]on[\s-]5|full[\s-]court\b/i, q: { id: 'q_5v5', label: 'Ready for full-court, 5v5 pace?', type: 'yesno' } },
  { test: /\btournament|bracket|playoffs?\b/i, q: { id: 'q_tourney', label: 'Played in a tournament or bracket before?', type: 'yesno' } },
  { test: /\bbeginner|new player|casual|no experience\b/i, q: { id: 'q_beginner', label: 'New to Practical Basketball? (totally fine either way)', type: 'yesno' } },
  { test: /\bscrim|squad|premade|team up\b/i, q: { id: 'q_squad', label: 'Signing up solo or with a squad?', type: 'select', options: ['Solo', 'With a squad'] } },
  { test: /\bmic|voice ?chat|\bvc\b|communication\b/i, q: { id: 'q_mic', label: 'Do you have a working mic for VC?', type: 'yesno' } },
  { test: /\bsub\b|substitute|bench\b/i, q: { id: 'q_sub', label: 'Open to a bench/sub role if needed?', type: 'yesno' } },
  { test: /\brank(ed)?\b|elo|leaderboard/i, q: { id: 'q_ranked', label: 'Played ranked/competitive modes before?', type: 'yesno' } },
];

function generateDynamicQuestions(description) {
  if (!description) return [];
  const seen = new Set();
  const qs = [];
  DYNAMIC_QUESTION_RULES.forEach(rule => {
    if (rule.test.test(description) && !seen.has(rule.q.id)) {
      seen.add(rule.q.id);
      qs.push(rule.q);
    }
  });
  ...
  return (
    <div className="rounded-xl overflow-hidden border" style={{ borderColor: C.line, background: C.panel }}>
      ...
      <div className="p-4 space-y-3">
        ...
        {data.dynamicAnswers && Object.keys(data.dynamicAnswers).length > 0 && (
          <div className="space-y-1 pt-1 border-t" style={{ borderColor: C.line }}>
            {Object.entries(data.dynamicAnswers).map(([id, val]) => {
              const rule = DYNAMIC_QUESTION_RULES.find(r => r.q.id === id);
              if (!rule || !val) return null;
              return (
                <div key={id} className="text-xs flex justify-between gap-2 pt-1.5" style={{ color: C.paintDim }}>
                  <span>{rule.q.label}</span>
                  <span style={{ color: C.paint }}>{val === 'yes' ? 'Yes' : val === 'no' ? 'No' : val}</span>
                </div>
              );
            })}
          </div>
...
/* ============ SIGNUP FLOW ============ */
const EMPTY_FORM = { eventId: '', photo: '', robloxUsername: '', discord: '', position: '', playstyle: '', passing: null, comp: null, availability: [], notes: '', dynamicAnswers: {} };
...
function SignupFlow({ openEvents, initialEventId, onSubmit, onCancel, submitting }) {
  ...
  const dynamicQuestions = useMemo(() => generateDynamicQuestions(selectedEvent ? selectedEvent.description : ''), [selectedEvent]);
  const setDynamicAnswer = (id, val) => setForm(f => ({ ...f, dynamicAnswers: { ...f.dynamicAnswers, [id]: val } }));
            ...
            {step === 3 && (
              <div className="space-y-6">
                ...
                {dynamicQuestions.length > 0 && (
                  <div className="space-y-5 pt-1 pb-1 border-t border-b" style={{ borderColor: C.line }}>
                    <div className="text-xs font-mono2 uppercase tracking-wide pt-4" style={{ color: C.rim }}>Specific To This Event</div>
                    {dynamicQuestions.map(q => (
                      <div key={q.id} className={q.id === dynamicQuestions[dynamicQuestions.length - 1].id ? 'pb-1' : ''}>
                        <label className="text-xs font-mono2 uppercase tracking-wide block mb-1.5" style={{ color: C.paintDim }}>{q.label}</label>
                        {q.type === 'yesno' ? (
                          <TilePicker options={[{ value: 'yes', label: 'Yes' }, { value: 'no', label: 'No' }]} value={form.dynamicAnswers[q.id]} onChange={v => setDynamicAnswer(q.id, v)} columns={2} />
                        ) : (
                          <TilePicker options={q.options.map(o => ({ value: o, label: o }))} value={form.dynamicAnswers[q.id]} onChange={v => setDynamicAnswer(q.id, v)} columns={2} />
                        )}
                      </div>
                    ))}
                  </div>
...
function EventForm({ initial, onSave, onCancel, busy }) {
  const [f, setF] = useState(initial || { title: '', date: '', time: '', location: '', capacity: '', description: '', photo: '' });
  const [photoError, setPhotoError] = useState('');
  const [photoLoading, setPhotoLoading] = useState(false);
  const fileInputRef = useRef(null);
    ...
    <div className="rounded-xl border p-5 space-y-4" style={{ borderColor: C.line, background: C.panel }}>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        ...
        <div className="sm:col-span-2">
          ...
          <div className="text-xs mt-1.5" style={{ color: C.paintDim }}>Mention things like 3v3, tournament, beginners welcome, or squads — signup adds matching questions automatically.</div>
        </div>
      </div>
...
function App() {
  ...
  const editEvent = async (id, f) => {
    const updated = events.map(e => e.id === id ? { ...e, title: f.title.trim(), date: f.date, time: f.time, location: f.location.trim(), capacity: f.capacity ? Number(f.capacity) : null, description: f.description.trim(), photo: f.photo || '' } : e);
