...
function PlayerCard({ data, compact = false, actions = null, questions = [] }) {
  ...
  const answeredQA = [];
  questions.forEach(q => {
    const a = data.customAnswers && data.customAnswers[q.id];
    if (a) {
      answeredQA.push({ text: q.text, a: a === 'yes' ? 'Yes' : a === 'no' ? 'No' : a });
      if (q.followUp && a === q.followUp.onAnswer) {
        const fa = data.customAnswers[q.id + '_followup'];
        if (fa) answeredQA.push({ text: q.followUp.text, a: fa });
      }
    }
  });
...
function ApplicantReviewMode({ applicants, questions, onAccept, onReject, onExit }) {
  ...
  return (
    <div className="rounded-xl border p-4 sm:p-6" style={{ borderColor: C.line, background: C.panel }}>
      ...
      <div key={current.id} className="max-w-sm mx-auto mb-6" style={{ animation: 'cardIn 0.3s ease-out' }}>
...
function EventManagerRow({ event, applicants, accepted, rejected, signupsById, onAccept, onReject, onReconsider, onAssignTeams, onDelete, onEdit, onToggleStatus, assigning }) {
  ...
  if (editing) {
    return <EventForm initial={{ ...event, capacity: event.capacity || '', questions: event.questions || [], askPosition: event.askPosition !== false, askPlaystyle: event.askPlaystyle !== false }} onSave={(f) => { onEdit(event.id, f); setEditing(false); }} onCancel={() => setEditing(false)} />;
  }
        ...
        <div className="border-t p-4 sm:p-5 space-y-6" style={{ borderColor: C.line }}>
          ...
          <div>
            ...
            {applicants.length === 0 ? (
              ...
              <ApplicantReviewMode applicants={applicants} questions={event.questions || []} onAccept={onAccept} onReject={onReject} onExit={() => setReviewMode(false)} />
            ) : (
