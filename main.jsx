...
/* ============ PLAYER CARD ============ */
function PlayerCard({ data, compact = false, actions = null, questions = [] }) {
  ...
  const answeredQA = (data.customAnswers || []).map((val, i) => ({ q: questions[i], a: val })).filter(x => x.q && x.a);
        ...
        <div className="min-w-0 flex-1">
          ...
          {answeredQA.length > 0 && (
            <div className="mt-1.5 space-y-0.5">
              {answeredQA.map((qa, i) => (
                <div key={i} className="text-xs" style={{ color: C.paintDim }}>
                  <span>{qa.q}: </span><span style={{ color: C.paint }}>{qa.a}</span>
                </div>
              ))}
            </div>
          )}
      ...
      <div className="p-4 space-y-3">
        <div>
          <div className="flex items-center gap-2">
            ...
            {archetype && (
              <div className="flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-semibold shrink-0" style={{ background: 'rgba(255,90,31,0.15)', color: C.rim }}>
        ...
        {answeredQA.length > 0 && (
          <div className="space-y-1.5 pt-1 border-t" style={{ borderColor: C.line }}>
            {answeredQA.map((qa, i) => (
              <div key={i} className="text-xs pt-1.5" style={{ color: C.paintDim }}>
                <div>{qa.q}</div>
...
function SignupFlow({ openEvents, initialEventId, onSubmit, onCancel, submitting }) {
  ...
  const eventQuestions = (selectedEvent && selectedEvent.questions) || [];
  const setCustomAnswer = (i, val) => setForm(f => {
  ...
  const canProceed = () => {
    ...
    if (step === 1) return !!(form.robloxUsername.trim() && form.position && form.playstyle);
                ...
                {eventQuestions.length > 0 && (
                  <div className="space-y-4 pt-1 pb-1 border-t border-b" style={{ borderColor: C.line }}>
                    ...
                    {eventQuestions.map((q, i) => (
                      <div key={i} className={i === eventQuestions.length - 1 ? 'pb-1' : ''}>
                        <label className="text-xs font-mono2 uppercase tracking-wide block mb-1.5" style={{ color: C.paintDim }}>{q}</label>
                        <input value={form.customAnswers[i] || ''} onChange={e => setCustomAnswer(i, e.target.value)} className="w-full rounded-lg px-4 py-3 border" style={{ background: C.panel, borderColor: C.line, color: C.paint }} />
                      </div>
                    ))}
                  </div>
                )}
...
function EventForm({ initial, onSave, onCancel, busy }) {
  const [f, setF] = useState(initial || { title: '', date: '', time: '', location: '', capacity: '', description: '', photo: '', questions: [] });
  ...
  return (
    <div className="rounded-xl border p-5 space-y-4" style={{ borderColor: C.line, background: C.panel }}>
      ...
      <div>
        ...
        <div className="text-xs mb-2" style={{ color: C.paintDim }}>Add anything else you want to ask players when they sign up for this event.</div>
        {questions.length > 0 && (
          <div className="space-y-2 mb-2">
            {questions.map((q, i) => (
              <div key={i} className="flex items-center gap-2 rounded-lg px-3 py-2" style={{ background: C.inkSoft }}>
                <span className="text-sm flex-1" style={{ color: C.paint }}>{q}</span>
        ...
        <div className="flex gap-2">
          <input value={newQuestion} onChange={e => setNewQuestion(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); addQuestion(); } }} placeholder="e.g. Do you have a mic for VC?" className="flex-1 rounded-lg px-3 py-2.5 border" style={{ background: C.inkSoft, borderColor: C.line, color: C.paint }} />
          <button type="button" onClick={addQuestion} className="px-3 rounded-lg border flex items-center gap-1 text-sm" style={{ borderColor: C.line, color: C.paint }}><Plus size={14} />Add</button>
        </div>
      </div>
...
function EventManagerRow({ event, applicants, accepted, rejected, signupsById, onAccept, onReject, onReconsider, onAssignTeams, onDelete, onEdit, onToggleStatus, assigning }) {
  ...
  if (editing) {
    return <EventForm initial={{ ...event, capacity: event.capacity || '', questions: event.questions || [] }} onSave={(f) => { onEdit(event.id, f); setEditing(false); }} onCancel={() => setEditing(false)} />;
  }
          ...
          <div>
            ...
            {applicants.length === 0 ? (
              ...
              <div className="space-y-2">
                {applicants.map(a => (
                  <PlayerCard key={a.id} data={a} compact questions={event.questions || []} actions={
...
function App() {
  ...
  const editEvent = async (id, f) => {
    const updated = events.map(e => e.id === id ? { ...e, title: f.title.trim(), date: f.date, time: f.time, location: f.location.trim(), capacity: f.capacity ? Number(f.capacity) : null, description: f.description.trim(), photo: f.photo || '', questions: f.questions || [] } : e);
